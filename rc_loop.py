...
def read_config(config_path):
    ...
    my_params["my_cen_cnt"]=num(ini.get('Proc','Centers_count'))
    my_params["my_cen_threshold"]=num(ini.get('Proc','Centers_threshold'))
    ...
    if check_read == True:
        ...
        print ('my_cen_cnt:%s' % (my_params["my_cen_cnt"]))
        print ('my_cen_threshold:%s' % (my_params["my_cen_threshold"]))
...
with open(sys.argv[1]) as list_in_file:
    for file in list_in_file:
        ...
        # SJM 2Apr add gen of new outfiles for center locations
        outfile_cendims = infilebasename.replace('tif','_cendims.dat')
        outfullpath_cendims = ('%s/%s' % (mp["my_outdir"], outfile_cendims))
        ...
        if outexistence.is_file():
            ...
            try:
                ...
                center_dims=center_find(holo,mp["my_cen_cnt"],mp["my_cen_threshold"])
                rec_vol = hp.propagate(holo, zstack)
                save(outfullpathhd5, rec_vol)

                # Write the center coords to a text file.
                cen_outfh= open(outfullpath_cendims, 'w')
                cen_outfh.write(str(center_dims))
                cen_outfh.close()
